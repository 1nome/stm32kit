...
void I2C_LCD_set(const uint8_t cmd){
	if(!I2C_LCD_addr){
		return;
	}
	I2C_LCD_busy();
...
void I2C_LCD_set_fast(const uint8_t cmd){
	I2C1_write((cmd & 0xF0) | 0x0C);
	//delay_us(1);
	I2C1_write((cmd & 0xF0) | 0x08);
...
void I2C_LCD_symbol(const uint8_t data){
	if(!I2C_LCD_addr){
		return;
	}
	I2C_LCD_busy();
...
void I2C_LCD_symbol_fast(const uint8_t cmd){
...
void I2C_LCD_print_own_addr(){
	char str[5];
	snprintf(str, 5, "0x%02X", I2C_LCD_addr);
	I2C_LCD_print(str);
}
